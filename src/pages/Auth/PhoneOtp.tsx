...
const PhoneOTP: React.FC = () => {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLIonInputElement>, index: number) => {
  ...
  return (
    <PageLayout
      header={<Header title="Phone OTP" type="page" showRightIcon={false} />}
    >
      <IonContent>
          ...
          <div className="flex flex-1 justify-center items-center px-4 py-8">
            <div className="w-full max-w-[400px] bg-white rounded-xl shadow-md px-6 py-8 flex flex-col items-start">
              ...
              <div className="self-center w-20 h-20 rounded-full bg-white border-2 border-[#222] flex items-center justify-center mb-4">
                <IonIcon icon={keyOutline} className="w-12 h-12" style={{ color: "#274C77" }} />
              </div>
              <h2 className="text-lg font-bold text-black mb-1font-inter">Enter your OTP Code</h2>
              <p className="text-xs  text-[#222] mb-4 max-w-xs font-inter">
                Check your phone for a new message and enter the OTP code below to proceed.
              </p>
              <form className="w-full flex flex-col gap-3 mt-3">
                <div className="flex justify-between items-center gap-2 mb-2 w-full" onPaste={handlePaste}>
                  {otp.map((data, idx) => (
                    <CustomInputForm
                      key={idx}
                      ...
                      onKeyDown={(e: KeyboardEvent<HTMLIonInputElement>) => handleKeyDown(e, idx)}
                      type="tel"
                      maxLength={1}
              ...
              <a href="#" className="text-sm font-medium text-[#0090FFFF] self-center mt-4 underline">
                Change Phone Number
              </a>
            </div>
          </div>
